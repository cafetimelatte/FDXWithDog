                ...
                <h3 class="area" style="height:60px">관리자 / 숙소 예약 수정 [예약번호 : ${b_info.b_id }]
                	<span>예약상태 : 
                		<select name="b_state" style="text-align:center;height:24px;padding:2px">
                			...
                			<option value="예약완료" <c:if test="${b_info.b_state eq '예약완료'}">selected</c:if>>예약완료</option>
                			<option value="예약취소" <c:if test="${b_info.b_state eq '예약취소'}">selected</c:if>>예약취소</option>
                			<option value="숙박완료" <c:if test="${b_info.b_state eq '숙박완료'}">selected</c:if>>숙박완료</option>
                		</select>
                	</span>
                	<span>
