...
!function($){
	...
	var a=/a/i,defs={
		...
		itemClass:'paging-item',sideClass:'paging-side',className:'jquery-paging',sideClass2:'paging-side',arrowNext:'',arrowPrev:'',
	...
	$.fn.paging=function(op){
		$(this).each(function(){
			...
			if(op.current>op.length){
				make(op,op.start-1,op.itemClass,op.prev);//prev button
                                make(op,1,op.itemClass,op.first);//first button
                                make(op,op.start-1,op.arrowPrev,op.sideClass);//next button
			}
			//pages button
			...
			if(op.current/op.length<op.max/op.length){
                            if(op.end<op.max) make(op,op.end+1,op.arrowNext,op.sideClass2);//next button
                            if(op.end<op.max) make(op,op.max,op.itemClass,op.last);//last button
                           if(op.end<op.max) make(op,op.end+1,op.itemClass,op.next);//next button
